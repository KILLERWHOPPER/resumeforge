...
export function SkeletonTable({
  ...
  return (
    <div className={cn('space-y-3', className)} {...props}>
      {/* Header */}
      <div className="flex gap-4 border-b border-border-light px-4 py-3 font-medium text-text-secondary">
      ...
      {Array.from({ length: rows }).map((_, rowIndex) => (
        <div key={rowIndex} className="flex gap-4 border-b border-border-light px-4 py-3">
          {Array.from({ length: columns }).map((_, colIndex) => (
            <Skeleton key={colIndex} variant="text" width="100%" height={16} className="flex-1" />
          ))}
        </div>
      ))}
    </div>
