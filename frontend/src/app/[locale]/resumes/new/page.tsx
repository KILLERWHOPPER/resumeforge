...
export default function NewResumePage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data: analysisData } = await api.post<JDAnalysis>(
        `/resumes/${data.id}/analyze-jd`
      );
    ...
    items.length > 0 ? (
      <div className="flex flex-wrap gap-2">
        {items.map((item, i) => (
          <span
            key={`${item}-${i}`}
            className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-primary-50 text-primary-700 dark:bg-primary-900/30 dark:text-primary-400"
          >
            {item}
          </span>
        ))}
    ...
    <div className="min-h-screen bg-background-primary">
      ...
      <main className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-text-secondary mt-1">{t("jdInput.subtitle")}</p>
        </div>
          ...
          <div
            className={`mb-6 flex items-start gap-3 rounded-xl border p-4 text-sm ${
              provider.source === "opencode_free"
                ...
                : "border-border-light dark:border-border-dark bg-background-secondary"
            }`}
          >
            <Sparkles className="h-4 w-4 mt-0.5 text-primary-600 dark:text-primary-400 flex-shrink-0" />
            <div>
              {provider.source === "opencode_free" ? (
              ...
              <Link
                ...
                className="mt-1 inline-block text-xs text-primary-600 dark:text-primary-400 hover:underline"
              >
                {tCommon("settings")}
              </Link>
            </div>
        ...
        {step === "form" && (
          <div className="bg-white dark:bg-neutral-900 border border-border-light dark:border-border-dark rounded-xl p-6">
            <div className="grid gap-5">
              <div>
                <label className="block text-sm font-medium text-text-label mb-1.5">
                  {t("jdInput.jdLabel")}
                ...
                <textarea
                  ...
                  className="w-full rounded-lg border bg-input-bg text-text-primary placeholder:text-input-placeholder p-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 border-input-border hover:border-input-border-hover focus-visible:ring-primary-500 focus-visible:ring-offset-white dark:focus-visible:ring-offset-neutral-950"
                />
                {formErrors.jd && (
              ...
              {experienceCounts && (
                <div className="flex items-start gap-3 rounded-lg bg-background-secondary dark:bg-background-tertiary border border-border-light dark:border-border-dark p-4">
                  <FileText className="h-4 w-4 mt-0.5 text-text-secondary flex-shrink-0" />
        ...
        {step === "analyzing" && (
          <div className="bg-white dark:bg-neutral-900 border border-border-light dark:border-border-dark rounded-xl p-10 text-center">
            <Loader2 className="h-8 w-8 text-primary-600 dark:text-primary-400 animate-spin mx-auto mb-4" />
            <p className="text-text-primary font-medium">{t("jdInput.analyzing")}</p>
            <p className="text-sm text-text-secondary mt-2">{t("generation.waiting")}</p>
            <div className="mt-6 h-1.5 w-full max-w-md mx-auto bg-neutral-100 dark:bg-neutral-800 rounded-full overflow-hidden">
              ...
              <div className="grid gap-5 sm:grid-cols-2">
                ...
                <div>
                  ...
                  {analysis.experience_level ? (
                    <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-neutral-100 text-neutral-700 dark:bg-neutral-800 dark:text-neutral-300">
                      {analysis.experience_level}
                    </span>
                  ) : (
                    <p className="text-sm text-text-tertiary">—</p>
        ...
        {step === "generating" && (
          <div className="bg-white dark:bg-neutral-900 border border-border-light dark:border-border-dark rounded-xl p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-sm text-text-secondary">{progressText || t("generation.waiting")}</p>
              </div>
            </div>
            ...
            <div className="mt-6 h-2 bg-neutral-100 dark:bg-neutral-800 rounded-full overflow-hidden">
              <div
            ...
            {livePreview && (
              <div className="mt-6 rounded-lg bg-background-secondary dark:bg-background-tertiary border border-border-light dark:border-border-dark p-4 max-h-48 overflow-auto">
                <p className="text-xs text-text-tertiary mb-2">{t("generation.livePreview")}</p>
