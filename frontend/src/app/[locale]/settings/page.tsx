          ...
          {mobileMenuOpen && (
            <div id="mobile-menu" className="md:hidden py-4 border-t border-border-light dark:border-border-dark animate-slide-in-from-top">
              <div className="flex flex-col gap-2">
                <Link href={`/${locale}/dashboard`} className="px-3 py-2 text-sm font-medium rounded-lg text-text-secondary hover:text-text-primary hover:bg-neutral-100 dark:hover:bg-neutral-800" onClick={() => setMobileMenuOpen(false)}>
                  {tCommon("dashboard")}
                </Link>
                <Link href={`/${locale}/experiences`} className="px-3 py-2 text-sm font-medium rounded-lg text-text-secondary hover:text-text-primary hover:bg-neutral-100 dark:hover:bg-neutral-800" onClick={() => setMobileMenuOpen(false)}>
                  {tCommon("experiences")}
                </Link>
                <Link href={`/${locale}/settings`} className="px-3 py-2 text-sm font-medium rounded-lg text-text-secondary hover:text-text-primary hover:bg-neutral-100 dark:hover:bg-neutral-800" onClick={() => setMobileMenuOpen(false)}>
                  {tCommon("settings")}
                </Link>
                <hr className="my-2 border-border-light dark:border-border-dark" />
                <button onClick={handleLogout} className="px-3 py-2 text-sm font-medium text-left text-error-600 dark:text-error-400 hover:bg-error-50 dark:hover:bg-error-900/20 rounded-lg">
                  {tAuth("logout")}
                </button>
              </div>
            </div>
...
function ProfileTab() {
  ...
  return (
    <div className="max-w-2xl">
      <h2 className="text-lg font-semibold text-text-primary mb-4">{t("title")}</h2>
      <div className="card bg-white dark:bg-neutral-900 border border-border-light dark:border-border-dark rounded-xl p-6">
...
function LLMConfigTab() {
  ...
  return (
    <div className="max-w-3xl">
      ...
      {showForm && (
        <div className="card bg-white dark:bg-neutral-900 border border-border-light dark:border-border-dark rounded-xl p-6 mb-8 animate-fade-in">
          <div className="grid gap-4 sm:grid-cols-2">
            <Input label={t("name")} placeholder={t("namePlaceholder")} value={name} onChange={(e) => setName(e.target.value)} required />
            <div>
              <Select
                label={t("provider")}
                value={provider}
            ...
            <Input label={t("baseUrl")} placeholder={t("baseUrlPlaceholder")} value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} required className="sm:col-span-2" />
            <Input label={t("apiKey")} placeholder={t("apiKeyPlaceholder")} value={apiKey} onChange={(e) => setApiKey(e.target.value)} type="password" required />
            <Input label={t("modelName")} placeholder={t("modelNamePlaceholder")} value={modelName} onChange={(e) => setModelName(e.target.value)} required />
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 mt-6">
            <Button variant="secondary" onClick={handleTestConnection} loading={testing} icon={<Globe className="h-4 w-4" />}>
      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {configs.map((config) => (
            <div
              key={config.id}
              className="bg-white dark:bg-neutral-900 border border-border-light dark:border-border-dark rounded-xl p-5"
            >
              <div className="flex items-start justify-between gap-2 mb-2">
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg text-text-tertiary hover:text-error-600 hover:bg-error-50 dark:hover:bg-error-900/20 transition-colors"
                  aria-label={tCommon("delete")}
                >
                  <Trash2 className="h-4 w-4" />
                </button>
              </div>
              <p className="text-sm text-text-secondary truncate mb-1">{config.model_name}</p>
              <p className="text-xs text-text-tertiary truncate mb-3">{config.base_url}</p>
              <div className="flex items-center justify-between pt-3 border-t border-border-light dark:border-border-dark">
                <span className="text-xs font-mono text-text-tertiary">{config.api_key_masked}</span>
                {!config.is_active && (
...
function SecurityTab() {
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="bg-white dark:bg-neutral-900 border border-border-light dark:border-border-dark rounded-xl p-6">
...
function AppearanceTab() {
  ...
  return (
    <div className="max-w-2xl">
      <h2 className="text-lg font-semibold text-text-primary mb-4">{t("title")}</h2>
      <div className="bg-white dark:bg-neutral-900 border border-border-light dark:border-border-dark rounded-xl p-6">
        <p className="text-sm font-medium text-text-label mb-3">{t("language")}</p>
